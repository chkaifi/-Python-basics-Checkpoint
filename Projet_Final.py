...
class EcoTrack:
    ...
    def setup_ui(self):
        ...
        self.btn_ajouter = tk.Button(frame_saisie, text="Ajouter la dépense", command=self.ajouter_depense, bg="#4CAF50", fg="white", font=("Arial", 10, "bold"))
        self.btn_ajouter.grid(row=3, columnspan=2, pady=10, sticky="we")
        ...
        self.btn_supprimer = tk.Button(frame_liste, text="Supprimer l'élément sélectionné", command=self.supprimer_selection, bg="#f44336", fg="white")
        self.btn_supprimer.pack(pady=10)

    
        self.lbl_total = tk.Label(self.root, text="Total des dépenses : 0.00 €", font=("Arial", 12, "bold"), fg="#2196F3")
    ...
    def supprimer_selection(self):
        """Supprime la ligne sélectionnée du tableau et du fichier CSV."""
        selection = self.tree.selection()
        
        if not selection:
            messagebox.showinfo("Info", "Veuillez d'abord sélectionner une ligne dans le tableau.")
            return

       
        reponse = messagebox.askyesno("Confirmation", "Voulez-vous vraiment supprimer cette dépense ?")
        ...
        if reponse:
            ...
            self.calculer_total()
            messagebox.showinfo("Succès", "Dépense supprimée avec succès.")

    def sauvegarder_etat_actuel(self):
        """Réécrit le fichier CSV à partir des données actuellement visibles dans le Treeview."""
        with open(self.filename, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["Date", "Description", "Catégorie", "Montant"])
            ...
            for item in self.tree.get_children():
                ...
                montant_net = str(valeurs[3]).replace(' €', '')
                writer.writerow([valeurs[0], valeurs[1], valeurs[2], montant_net])
    ...
    def calculer_total(self):
        total = 0.0
        for item in self.tree.get_children():
            valeur_str = self.tree.item(item)['values'][3]
            total += float(valeur_str.replace(' €', '').replace(',', '.'))
